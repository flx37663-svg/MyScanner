...
class Crawler:
    def __init__(self, entry_url, max_depth=2, max_pages=30):
        self.entry_url, self.domain = entry_url.strip().rstrip('/'), urlparse(entry_url).netloc
        self.max_depth, self.max_pages = max_depth, max_pages
        self.visited, self.discovered = set(), set()
    ...
    def is_static(self, url):
        exts = ['.jpg','.jpeg','.png','.gif','.css','.js','.woff','.ttf','.pdf','.ico','.zip']
    ...
    def start(self):
        ...
        while todo and len(self.discovered) < self.max_pages:
            ...
            if urlparse(url).netloc != self.domain or self.is_static(url): continue
            try:
